...
public class Main {
  ...
  private static volatile boolean hasDetectedRedelivery =
      false; // detected any messages being redelivered?
  private static volatile boolean isShutdown = false; // are we done?

  /**
   * This is the main app. Use this type of app for receiving Guaranteed messages (e.g. via a queue
   * endpoint).
   */
  public static void main(String[] args) throws InterruptedException, IOException {
    ...
    final MessagingService messagingService =
        MessagingService.builder(ConfigurationProfile.V1).fromProperties(properties).build();
    messagingService.connect(); // blocking connect
    final PersistentMessageReceiver receiver =
        messagingService
    ...
    try {
      ...
      System.err.printf(
          "%n*** Could not establish a connection to queue '%s': %s%n", QUEUE_NAME, e.getMessage());
    ...
    receiver.receiveAsync(
        message -> {
          ...
          if (message.isRedelivered()) { // useful check
            // this is the broker telling the consumer that this message has been sent and not ACKed
            // before.
            // this can happen if an exception is thrown, or the broker restarts, or the network
            // disconnects
